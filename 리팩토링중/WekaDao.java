public class WekaTagDao{
  ...
  public void setUp(){
    this.c = DriverManager.getConnection("jdbc:mysql://localhost/instadata?autoReconnect=true&useSSL=false","root","song9207108");
  ...
  public Instances getInstances(){
    ...
    rs.last();
    Tag tags[] = new Tag[rs.getRow()];  //getRow() : Retrieves the current row number.
    rs.first();
    ...
    {
      String attributeString = (i+1)+"."+tags[i].getTagName();  //태그이름들을 1.태그 이런식으로 다시 저장
      attributes[i] = new Attribute(attributeString);
    }
    
    //웨카 라이브러리에서 제공하는 클래스.
    FastVector finalAttribute = new FastVector(attributes.length);  //숫자를 주면 이만큼 용량이 생김.
    for(i = 0; i<attributes.length;i++)
    {
